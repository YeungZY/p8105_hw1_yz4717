...
As the former analysis mentioned, logical and numeric vector can convert into numeric one without any difficulty just because they are actual numbers in the system. for the same reason, the system can't convert character vector. When it comes to factor vector, if we force R to do a type conversion, the system will convert them into real numbers.
